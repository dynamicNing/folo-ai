    ...
    <template v-else>
      ...
      <div class="section">
        ...
        <div class="recent-list">
          ...
          <div v-if="!recent.length" class="empty" style="padding:2rem"><p>暂无已发布内容</p></div>
        </div>
      </div>
    </template>
  </div>
...
onMounted(async () => {
  try {
    ...
    allItems.value.sort((a, b) => new Date(b.date || 0).getTime() - new Date(a.date || 0).getTime())
...
.stats-row { display: grid; grid-template-columns: repeat(4, 1fr); gap: 1px; background: var(--border); border: 1px solid var(--border); border-radius: var(--radius); overflow: hidden; margin-bottom: 2.5rem; }
.stat { background: var(--bg-card); padding: 1.5rem 1.25rem; text-align: center; }
.stat-num { font-family: var(--font-display); font-size: 2.25rem; font-weight: 700; letter-spacing: -0.04em; color: var(--accent); line-height: 1; margin-bottom: 0.4rem; }
...
.recent-list { border: 1px solid var(--border); border-radius: var(--radius); background: var(--bg-card); overflow: hidden; }
.recent-row { display: flex; align-items: center; gap: 1rem; padding: 0.9rem 1.25rem; border-bottom: 1px solid var(--border); text-decoration: none; color: inherit; transition: background 0.15s; }
