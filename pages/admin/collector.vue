  ...
  <div class="page">
    <div class="page-header">
      ...
      <div class="header-actions">
        <button class="btn btn-ghost btn-sm" :disabled="loading.status" @click="loadStatus">
          <span v-if="loading.status">⟳</span><span v-else>↻</span>
        </button>
      </div>
    </div>
    ...
    <div class="platforms-row">
      <div v-for="(info, name) in status.platformLastItems" :key="name" class="platform-chip" :class="name">
        <span class="chip-emoji">{{ platformEmoji[name as PlatformKey] }}</span>
        <span class="chip-name">{{ platformNames[name as PlatformKey] }}</span>
    ...
    <div class="section">
      ...
      <div class="collect-buttons">
        ...
        >
          <span v-if="collecting === 'all'" class="spin">⟳</span><span v-else>🚀</span>
      ...
      <div v-if="collectResult" class="success-msg">
        ...
        <span v-for="(r, name) in collectResult" :key="name">
          {{ platformNames[name as PlatformKey] }} {{ r.saved }}条{{ r.error ? '⚠️' : '✅' }}
        </span>
      </div>
    </div>
    ...
    <div class="section">
      ...
      <div v-if="loading.items" class="loading-state"><span class="spin large">⟳</span> 加载中…</div>
      ...
      <div v-else class="items-list">
        <div v-for="item in items" :key="item.url + item.fetched_at" class="item-card" @click="openUrl(item.url)">
          <div class="item-header">
            <span class="item-platform" :class="item._platform">{{ platformEmoji[(item._platform || '') as PlatformKey] }} {{ platformNames[(item._platform || '') as PlatformKey] }}</span>
          ...
          <div class="item-footer"><span class="item-link">🔗 {{ truncate(item.url, 50) }}</span></div>
        </div>
      </div>
...
import type { SocialItem, SocialStatus, CollectResult } from '~/types/article'

definePageMeta({ layout: 'admin', middleware: 'auth' })

type PlatformKey = 'youtube' | 'weibo' | 'tech' | string

const platformEmoji: Record<string, string> = { youtube: '▶️', weibo: '🐦', tech: '📰' }
const platformNames: Record<string, string> = { youtube: 'YouTube', weibo: '微博', tech: '科技资讯' }
const platforms = [
  { id: 'youtube', name: 'YouTube', emoji: '▶️' },
  { id: 'weibo', name: '微博', emoji: '🐦' },
  { id: 'tech', name: '科技资讯', emoji: '📰' },
]
const tabs = [{ id: 'all', name: '全部', emoji: '🌐' }, ...platforms]

const api = useApi()
const status = ref<SocialStatus>({ rsshubStatus: 'unknown', lastRun: null, platformLastItems: {} })
...
const missingPlatforms = computed(() =>
  platforms.map(p => p.id).filter(id => !status.value.platformLastItems[id])
)
...
const formatShortTime = formatTime
...
async function loadStatus() {
  ...
  try { status.value = await api.getSocialStatus() }
  catch (e) { console.error('status error', e) }
  finally { loading.value.status = false }
}
...
function switchTab(tab: string) { activeTab.value = tab; loadItems(1) }
...
async function collectOne(platform: string) {
  ...
  try {
    ...
    const res = await api.triggerCollect(platform, auth.authHeader())
    collectResult.value = res
    await loadStatus()
    await loadItems(1)
  } catch (e) {
    collectError.value = (e as Error).message
  } finally {
...
onMounted(() => { loadStatus(); loadItems() })
</script>
...
.status-card { display: flex; align-items: center; gap: 0.75rem; background: var(--bg-raised); border: 1px solid var(--border); border-radius: var(--radius); padding: 0.875rem 1.25rem; min-width: 160px; }
...
.platform-chip { display: flex; align-items: center; gap: 0.4rem; background: var(--bg-raised); border: 1px solid var(--border); border-radius: 20px; padding: 0.35rem 0.75rem; font-size: 0.8rem; }
...
.collect-btn { display: flex; align-items: center; gap: 0.4rem; padding: 0.55rem 1rem; border-radius: var(--radius-sm); border: 1px solid var(--border); background: var(--bg-raised); font-size: 0.85rem; font-weight: 600; cursor: pointer; transition: all 0.15s; }
...
.tab-btn { padding: 0.35rem 0.75rem; border-radius: var(--radius-sm); border: 1px solid var(--border); background: transparent; font-size: 0.8rem; font-weight: 600; cursor: pointer; transition: all 0.15s; }
...
.loading-state, .empty-state { text-align: center; padding: 3rem 1rem; color: var(--text-muted); font-size: 0.9rem; }
.empty-icon { font-size: 2.5rem; margin-bottom: 0.5rem; }
.empty-hint { font-size: 0.8rem; margin-top: 0.5rem; }

.items-list { display: flex; flex-direction: column; gap: 0.6rem; }
.item-card { background: var(--bg-raised); border: 1px solid var(--border); border-radius: var(--radius); padding: 0.875rem 1rem; cursor: pointer; transition: all 0.15s; }
.item-card:hover { border-color: var(--accent); background: var(--bg); }
.item-header { display: flex; align-items: center; gap: 0.5rem; flex-wrap: wrap; margin-bottom: 0.4rem; font-size: 0.72rem; }
